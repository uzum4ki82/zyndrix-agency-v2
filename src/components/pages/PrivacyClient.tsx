...
export default function PrivacyClient({ dict, locale }: { dict: any, locale: string }) {
  const icons = [Eye, Lock, Database];
  const sideIcons = [Shield, Lock, Database];
    ...
    <div className="min-h-screen bg-black text-white font-body selection:bg-primary selection:text-white">
      ...
      <main className="relative z-10 pt-60 pb-40 px-6">
        <div className="max-w-4xl mx-auto">
          <SectionHeader title={dict.privacy_page.title} subtitle={dict.privacy_page.subtitle} />
          
          <div className="space-y-24 mt-40">
            {dict.privacy_page.sections.map((section: any, i: number) => {
